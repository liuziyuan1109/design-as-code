...
def generate_image(client, user_prompt, layer_id, save_folder):
    ...
    # user_prompt = '''Create a 1059 × 1059 px layer featuring a single, bold right-pointing chevron ( “>” ) shape with thick, straight arms meeting at a sharp 90-degree angle. Fill the chevron with solid red (RGB 239 66 71 / #EF4247) at 100 % opacity. All areas outside the chevron are fully transparent.'''

    # user_prompt = '''Create a 1080 × 1920 solid rectangular background filled with a soft, muted blush-pink color (hex #F2D9D6, rgba 242 217 214 1) at full opacity'''
...
def edit_image(client, user_prompt, layer_id, save_folder):
    image_url = f"generated_images/edited_layer{layer_id}.png"
    temp_image_url = f"generated_images/edited_temp_layer{layer_id}.png"
    folder = save_folder
    os.makedirs(os.path.join(folder, "generated_images"), exist_ok=True)
